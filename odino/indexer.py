...
from .utils import chunk_text, format_file_size, is_text_file, load_config, load_ignore_patterns, should_ignore_file
...
class Indexer:
    ...
    def _fallback_to_cpu(self) -> None:
        ...
        try:
            ...
            self.console.print("[yellow]Switching to CPU for memory efficiency...[/yellow]")
            # Reload model on CPU
    ...
    def discover_files(self, target_path: Path) -> List[Path]:
        ...
        for file_path in target_path.rglob('*'):
            if file_path.is_file() and is_text_file(file_path):
                if not should_ignore_file(file_path, self.project_root, self.ignore_patterns):
    ...
    def index_files(self, target_path: Path, force: bool = False, progress_callback: Optional[Callable[[str], None]] = None) -> Dict[str, float]:
        ...
        if force:
            ...
            for file_path in files:
                ...
                if (relative_path not in indexed_files or 
                    indexed_files[relative_path] != current_hash):
        ...
        self.console.print(f"Generated {len(all_chunks)} chunks from {len(files_to_index)} files")
        ...
        batch_size = self.config.get('embedding_batch_size', 32 if self._is_mps_available() else 1000)
            ...
            for i in tqdm(range(0, len(texts), batch_size)):
                ...
                try:
                    ...
                    if "out of memory" in str(e).lower() or "MPS" in str(e):
                        self.console.print(f"[yellow]MPS memory error in batch {i//batch_size + 1}, falling back to CPU...[/yellow]")
        ...
        self.console.print(f"[green]Successfully indexed {len(files_to_index)} files![/green]")
    ...
    def index(self, force: bool = False, progress_callback: Optional[Callable[[str], None]] = None) -> Dict[str, float]:
        """Convenience method to index the project root."""
        return self.index_files(self.project_root, force=force, progress_callback=progress_callback)